...
export function DashboardScreen({
  ...
  const [netWorthChartSize, setNetWorthChartSize] = useState({
    width: 0,
    height: 0,
  });

  useEffect(() => {
    const element = netWorthChartRef.current;
    if (!element) return;

    const updateSize = () => {
      const rect = element.getBoundingClientRect();
      setNetWorthChartSize({
        ...
        <div className="bg-background/60 backdrop-blur-md border border-divider/40 rounded-2xl shadow-sm overflow-hidden">
          ...
          <div ref={netWorthChartRef} className="h-[180px] w-full min-w-0">
            {accounts.length === 0 ? (
              ...
              <LineChart
                width={netWorthChartSize.width}
                height={netWorthChartSize.height}

...
export function AccountsScreen({
  ...
  const grouped = Object.entries(typeConfig).reduce<
    Record<
      string,
      { config: (typeof typeConfig)[string]; accounts: typeof accounts }
    >
  >((acc, [type, config]) => {
  ...
  const handleDeleteAccount = async (id: number) => {
    if (
      !confirm(
        "Are you sure you want to remove this account? Transactions will be preserved if they belong to other active accounts.",
      )
    )
      return;
    setIsDeleting(id);
  ...
  const handleDisconnectBank = async (plaidItemId: number | null) => {
    ...
    if (
      !confirm(
        "Disconnect this institution? ALL associated accounts and transactions will be permanently deleted.",
      )
    )
      return;
          ...
          <CardContent className="px-0 py-0">
            <div className="flex flex-col divide-y divide-divider/50">
              {connections.map((conn) => (
                <div
                  key={conn.id}
                  className="flex justify-between items-center px-6 py-4 hover:bg-default-50/50 transition-all relative"
                >
                  <div className="flex flex-col gap-0.5">
                    ...
                    <span className="text-xs text-default-400">
                      {conn.accountCount} account
                      {conn.accountCount !== 1 ? "s" : ""}
                      {conn.lastSyncedAt
          ...
          <CardContent className="flex flex-col items-center justify-center py-20 gap-4 text-center">
            ...
            <div className="flex flex-col gap-1">
              ...
              <p className="text-sm text-default-400 max-w-xs">
                Connect your bank via Plaid to securely import your transactions
                and balances.
              </p>
            </div>
          </CardContent>
            ...
            return (
              ...
              >
                <CardHeader className="flex justify-between items-center px-6 pt-5 pb-3">
                  ...
                  <div
                    className={`font-bold text-xl tracking-tight ${config.isDebt ? "text-danger" : "text-success"}`}
                  >
                ...
                <CardContent className="px-0 py-0">
                  <div className="flex flex-col divide-y divide-divider/50">
                    {accs.map((acc) => (
                      <div
                        key={acc.id}
                        className="flex justify-between items-center px-6 py-5 hover:bg-default-50/50 transition-all cursor-pointer group relative"
                      >
                        <div className="flex flex-col gap-0.5">
                              ...
                              <DropdownMenu aria-label="Account actions">
                                ...
                                {acc.plaidItemId ? (
                                  <DropdownItem
                                    ...
                                    className="text-danger"
                                    onAction={() =>
                                      handleDisconnectBank(acc.plaidItemId)
                                    }
                                  >
                                    <div className="flex flex-col gap-0.5">

...
import { runAICategorization } from "../../server/plaid.sync.fns";
...
export function ReviewTable({
  ...
  const visibleTxns = (
    showAll ? transactions : transactions.filter((tx) => !tx.isReviewed)
  ...
  const allSelected =
    ...
    allTransactionIds.every((id) => selected.has(id));
  ...
  const getParentGroupId = (catId: number | null | undefined) => {
    ...
    return (
      categories.find((g) => g.children.some((c) => c.id === catId))?.id ??
      categories[0]?.id ??
      null
    );
  };
  ...
  const getCategoryColor = (catId: number | null | undefined) => {
    ...
    const idx = categories.findIndex((g) =>
      g.children.some((c) => c.id === catId),
    );
          ...
          {txns.map((tx) => {
            ...
            return (
              ...
              >
                ...
                <button
                  className="ml-2 text-default-300 hover:text-default-500 transition-colors shrink-0 cursor-pointer"
                  onClick={(e) => e.stopPropagation()}
                >
                  <TagIcon size={13} />
                </button>
                            ...
                            <div key={g.id} className="px-1.5 pb-1.5">
                              ...
                              {g.children.map((c) => (
                                <button
                                  key={c.id}
                                  onClick={() =>
                                    handleCategoryChange(tx.id, c.id)
                                  }
                                  className={`group w-full text-left px-2.5 py-2 text-sm rounded-xl flex items-center gap-2.5 transition-all cursor-pointer border ${
                                    tx.category?.id === c.id
                ...
                <span
                  className={`ml-3 text-sm font-semibold tabular-nums w-20 text-right shrink-0 ${tx.amount < 0 ? "text-danger" : "text-foreground"}`}
                >
                  {tx.amount < 0 ? "+" : ""}
            ...
            <ModalDialog className="overflow-hidden rounded-3xl border border-divider bg-content1 shadow-2xl text-foreground p-0 max-w-xl">
              ...
              <ModalBody className="px-7 py-6 flex flex-col gap-5">
                ...
                <div className="flex flex-col gap-2">
                  <label className="text-2 font-semibold text-default-500">
                    Group
                  </label>
                  <select
